...
def download_cv_from_api(cv_id: str, token: str) -> Optional[Dict[str, any]]:
    metadata_url = f"https://partnersapi.applygateway.com/api/Candidates/getcv/{cv_id}"
    headers = {
    ...
    try:
        ...
        r = requests.get(metadata_url, headers=headers, timeout=30)
        r.raise_for_status()
        json_data = r.json()
        
        # Step 2: Extract the file download URL
        file_url = json_data.get("data", {}).get("url")
        if not file_url:
            logger.error(f"CV file URL not found for cVid {cv_id}")
            return None

        # Step 3: Fetch the actual file from cvfilemanager
        ...
        logger.error(f"Error downloading CV for cVid {cv_id}: {e}")
        return None
